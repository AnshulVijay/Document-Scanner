...
pts = np.argwhere(edged_image>0)
y1,x1 = pts.min(axis=0)
y2,x2 = pts.max(axis=0)

cropped = image[y1:y2, x1:x2]
cv2.imwrite("edge_detection.jpg", cropped)
edge_detected = cv2.rectangle(image.copy(), (x1,y1), (x2,y2), (0,255,0), 2, cv2.LINE_AA)
cv2.imshow("edge_detected", edge_detected)
end = time.time()
...
sizetransformed = imutils.resize(transformed, height = 300)
